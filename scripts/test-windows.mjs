...
try {
  ...
  if (!response?.ok) throw new Error(`The Windows server did not respond on test port ${testPort}.`)
  const page = await fetch(`http://127.0.0.1:${testPort}/`)
  const html = await page.text()
  if (!page.ok || !html.includes('VPZONE Control')) throw new Error('The embedded interface is missing from the executable.')
  ...
  console.log(`Windows test passed: API ${response.status}, interface ${page.status}, PID ${child.pid}, resources ${metrics}`)
} finally {
  child.kill()
}
